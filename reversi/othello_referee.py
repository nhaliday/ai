...
def show(board, player_one, player_two, one=0, two=0, player=black, wins=None):			#now with ANSI power!
	system('clear')
	...
	for row in range(10, 90, 10):
		...
		for col in range(1, 9):
			...
			if (row/10+col)%2==0:            # Torbert, 1.5.2010
				val='42'
			else:
				val='44'
			val='42'
...
#	print 'White:',count(board,white),' '
	print 'Black:',count(board,black),     # Torbert, 1.5.2010
	print '[%0.2f]'%(time_pool-one),
	if player==black:
		print '*'
	else:
		print ' '
	print 'White:',count(board,white),   
	print '[%0.2f]'%(time_pool-two),
	if player==white:
		print '*'
	else:
		print ' '
...
# rd signals to return the differential, not the winning player
	...
	while squares < 64 and stuck < 2:
		...
		player = opponent_color(player)
		if flag:
			show(board, player_one, player_two, one, two, player, wins=tw)
	w, b = count(board, white), count(board, black)
	#if flag:
